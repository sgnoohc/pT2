...
int main(int argc, char **argv)
{
    ...
    //-----------------------------------------------------------------------------------------------------------------------------------------
    // Initialization of various things (Pixel map, reading files, preparing output files
    //-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    try
    {
        loadSuperbinDetIdMap(pixelMapFileDir, superbinToDetIds_POS, superbinToDetIds_NEG, superbinToDetIds_NON);
    }
    ...
    for (Long64_t ievt = 0; ievt < entriesToProcess; ++ievt)
    {
        ...
        if (ievt % 2 == 0 || ievt == entriesToProcess) printProgressBar(ievt, entriesToProcess);
        ...
        std::cout << " " << std::endl;
        std::cout << " " << std::endl;
        std::cout << " pt2s.size(): " << pt2s.size() << std::endl;
        ...
        for (auto &pt2 : pt2s)
        {
            ...
            if (not (reader.pls_pt->at(plsIdx) > 0.6 and reader.pls_pt->at(plsIdx) < 0.8))
            {
                continue;
            }
